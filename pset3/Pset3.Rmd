...
# save predictions of model into new data frame with variable we want to plot against
below_pred <- data.table(hrs_82_bins_dist_pred = predict(below_lm, data.frame(hrs_82_bins_dist, bins.midpoint)[1:19,]), bins.midpoint = bins.midpoint[1:19])
...
\bigskip
From our histogram, it does not appear visually that there is a discontinuity in the distribution of 1982 HRS scores around the 28.5 threshold. We further test this by running local linear regressions of the density of each bin by the midpoints of each bin on either side of the threshold and conclude that the relationship between bin midpoint and density of the midpoint is close to zero on either side of the threshold. This suggests that manipulation of 1982 HRS scores to game the assignment mechanism is not a concern in this context. 
