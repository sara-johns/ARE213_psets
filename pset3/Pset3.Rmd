...
data[,hrs_min_thresh := hrs_82 - 28.5]
data[,hrs_min_thresh_in := hrs_min_thresh * above_28pt5]
```

We can write the first stage as
$$
NPL_i = \delta_0 + \delta_1 1(HRS_i \ge 28.5) + \delta_2 (HRS_i - c) + \delta_3 (HRS_i - c) 1(HRS_i \ge 28.5) + \gamma X_i + \nu_i
$$
where the instrument for NPL status is whether HRS is above 28.5, we control for the running variable and other covariates, and we allow the slopes to vary on either side of the threshold. Now we estimate this first stage regression, including the full set of controls used in the fourth regression in question 1. 

```{r}
first_stage <- feols(npl2000 ~ above_28pt5 + hrs_min_thresh + hrs_min_thresh_in + lnmeanhs8_nbr + tothsun8_nbr + ownocc8_nbr + firestoveheat80_nbr +
...
$$ P_{i} = \beta_0 + \beta_1 \hat{NPL_i} + \beta_2 (HRS_i - c) + \beta_3 (HRS_i - c)1(HRS_i \ge 28.5) + \psi X_i + \varepsilon_i$$
where $P_i$ is the logged median housing price in 2000, $\hat{NPL_i}$ are the fitted values from the first stage, and the set of covariates is the same as in the first stage. The two assumptions for valid IV estimation are: 1) $Cov(z_i, d_i) \neq 0$ (relevance of the instrument) which our first stage showed we satisfy, and 2) $Cov(z_i, \epsilon_i)$ (exogeneity) which the facts and discussion in question 2 suggest we satisfy. We estimate the 2SLS regression for the full dataset and for a subset of census tracts with HRS scores between 16.5 and 40.5. 

```{r}
iv_reg_full <- iv_robust(lnmdvalhs0_nbr~ npl2000 + hrs_min_thresh + hrs_min_thresh_in + lnmeanhs8_nbr + tothsun8_nbr + ownocc8_nbr + firestoveheat80_nbr +
            noaircond80_nbr + nofullkitchen80_nbr + zerofullbath80_nbr + bedrms0_80occ_nbr + bedrms1_80occ_nbr +
            bedrms2_80occ_nbr + bedrms3_80occ_nbr + bedrms4_80occ_nbr + blt0_1yrs80occ_nbr +
            blt2_5yrs80occ_nbr + blt6_10yrs80occ_nbr + blt10_20yrs80occ_nbr + blt20_30yrs80occ_nbr + 
            blt30_40yrs80occ_nbr + detach80occ_nbr + attach80occ_nbr + 
...
```{r}
iv_reg_narrow <- iv_robust(lnmdvalhs0_nbr~ npl2000 + hrs_min_thresh + hrs_min_thresh_in + lnmeanhs8_nbr + tothsun8_nbr + ownocc8_nbr + firestoveheat80_nbr +
            noaircond80_nbr + nofullkitchen80_nbr + zerofullbath80_nbr + bedrms0_80occ_nbr + bedrms1_80occ_nbr +
            bedrms2_80occ_nbr + bedrms3_80occ_nbr + bedrms4_80occ_nbr + blt0_1yrs80occ_nbr +
            blt2_5yrs80occ_nbr + blt6_10yrs80occ_nbr + blt10_20yrs80occ_nbr + blt20_30yrs80occ_nbr + 
            blt30_40yrs80occ_nbr + detach80occ_nbr + attach80occ_nbr + 
