...
traffic_TU[state == 99, state_name := "TU"]
...
```{r}
traffic_prep <- dataprep(foo = traffic_TU,
...
synth.out <- synth(traffic_prep)
```

```{r}
## plot in levels (treated and synthetic)
path.plot(dataprep.res = traffic_prep,synth.res = synth.out)

## plot the gaps (treated - synthetic)
gaps.plot(dataprep.res = traffic_prep,synth.res = synth.out)
```
