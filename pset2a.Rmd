...
$$
In the second step, we regress $\tilde{y}_{it}$ and $\tilde{X}_{it}$ on $\tilde{R}_{it}$ and obtain the residuals, $\ddot{y}_{it}$ and $\ddot{X}_{it}$. The FWL theorem then guarentees that a regression of $\ddot{y}_{it}$ on $\ddot{X}_{it}$ will yield the fixed effects estimate of $\beta$. 
...
In the case of unbalanced panel data, 
