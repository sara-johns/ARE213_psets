...
When we calculate the White-robust and clustered standard errors by hand they are quite similar to the standard errors calculated by the "canned" commands above. 
\bigskip
...
```{r}
fixed <- plm(ln_fat_pc ~ primary, data = traffic, index = c("state", "year"), model = "within")
summary(fixed)
coeftest(fixed, vcov = vcovHC(fixed, type = "sss", cluster = "group"))
```
...
fixed_cov <- plm(ln_fat_pc ~ primary + secondary + college + 
                beer + ln_unemploy + ln_totalvmt + ln_precip +
                ln_snow + rural_speed + urban_speed, data = traffic, index = c("state", "year"), model = "within")
summary(fixed_cov)
coeftest(fixed_cov, vcov = vcovHC(fixed, type = "sss", cluster = "group"))
```
\bigskip
...
summary(fived, se = "standard")

# Estimate long differences estimator with covariates
ld <- lm(ln_fat_pc_ld ~ primary_ld + secondary_ld + college_ld + beer_ld + ln_unemploy_ld + ln_totalvmt_ld + ln_precip_ld + ln_snow_ld + rural_speed_ld + urban_speed_ld -1, data = traffic[year == 2003])
summary(ld) 
